...
public class Tree_HotelReviews {
    ...
    public static boolean searchString(String pattern, Node root){
        if (pattern.length() < 1) {
            return false;
        }
    ...
    public static void insertString(String s, Node root){
        Node temp = root;
        //assuming everything is lowercase
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);
            int cindex = (int) (c);
            if (temp.ptrs[cindex] == null) {
                Node newNode = new Node(c);
                temp.ptrs[cindex] = newNode;
                temp = newNode;
            } else {
                temp = temp.ptrs[cindex];
            }
        }
    }

    public static void addToMap(HashMap<Integer, ArrayList<Integer>> hashMap, int matches, int rindex){
        if (!hashMap.containsKey(matches)){
            ArrayList<Integer> temp = new ArrayList<>();
            temp.add(rindex);
            hashMap.put(matches, new ArrayList<>(temp));
        } else {
            ArrayList<Integer> temp = hashMap.get(matches);
            temp.add(rindex);
    ...
    public static ArrayList<Integer> solve(String A, ArrayList<String> B) {
        ...
        for (int i = 0; i < B.size(); i++) {
            ...
            addToMap(hashMap, matches, i);
        }

        for (int i = maxMatches; i >= minMatches; i--) {
            if (hashMap.containsKey(i)){
                ArrayList<Integer> tempi = hashMap.get(i);
                ans.addAll(tempi);
            }
        }
